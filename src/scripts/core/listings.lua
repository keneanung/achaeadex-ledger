...
function listings.list_designs(state, opts)
  ...
  for _, design in pairs(state.designs or {}) do
    if (not opts.type or design.design_type == opts.type)
      and (not opts.provenance or design.provenance == opts.provenance)
      and (opts.recovery == nil or design.recovery_enabled == opts.recovery) then
      table.insert(rows, {
        ...
        design_remaining = design.capital_remaining or 0
      })
    end
  end
...
function listings.list_items(state, opts)
  ...
  for _, item in pairs(state.crafted_items or {}) do
    ...
    local unresolved = item.design_id == nil

    if (not opts.design or item.design_id == opts.design)
